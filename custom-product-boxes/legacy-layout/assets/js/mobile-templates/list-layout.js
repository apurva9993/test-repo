...
(function($, window, document) {
     ...
     */
     cpbMobileListLayout.cpbMobileLayout = function() {
        ...
        if (wdm_bundle_params.isProductCPBSubscription) {
            this.totalProductPrice = add_signup_fee(this.totalProductPrice);
        }
        ...
        this.layoutSelector.on('change', '#cpb_main_qty_mobile', {
            cpbMobileLayout: this
        }, this.mainProductQtyChange);
    ...
    cpbMobileListLayout.cpbMobileLayout.prototype.mainProductQtyChange = function() {
        jQuery('#cpb_main_qty_desk').val(jQuery('#cpb_main_qty_mobile').val());
        // event.data.cpbMobileLayout.displayTotalPrice();
        // var currentPrice = this.totalProductPrice * jQuery('#cpb_main_qty_mobile').val();
        // currentPrice = wdm_get_price_format(currentPrice);
        var reg_price = jQuery('#main_reg_price').val() * jQuery('#cpb_main_qty_mobile').val();
        reg_price = wdm_get_price_format(reg_price);
        // jQuery('div.wdm_product_info .price').find( "ins .amount" ).html( currentPrice );
        // jQuery('div.wdm_product_info .price').find( ".amount" ).html( currentPrice );
        
        if (cpbMobileListLayout.cpbProductData.isDynamicPricingEnabled != 'yes') {
            cpbMobileListLayout.mobileLayoutSelector.find('.wdm_bundle_price').find( "p.price > del > span.woocommerce-Price-amount.amount .amount" ).html(reg_price);
            // jQuery('div.wdm_product_info .price').find( "del .amount" ).html( reg_price );
        }
    ...
    cpbMobileListLayout.cpbMobileLayout.prototype.displayTotalPrice = function() {
        var cartPrice = this.totalProductPrice * jQuery('#cpb_main_qty_mobile').val();
        var totalPrice = substract_signup_fee(cartPrice);
        cartPrice = wdm_get_price_format(cartPrice);
        totalPrice = wdm_get_price_format(totalPrice);


        this.layoutSelector.find('.wdm_bundle_price').find( "p.price > span.woocommerce-Price-amount.amount" ).html(totalPrice);
        // jQuery('div.wdm_product_info .price').find( "ins .amount" ).html( currentPrice );
        // jQuery('div.wdm_product_info .price').find( ".amount" ).html( currentPrice );
        snackbar(cpbMobileListLayout.cpbProductData.numberOfProductsInBox + ' / ' + cpbMobileListLayout.cpbProductData.boxQuantity + ' ' + mobileListLayoutParams.productsAddedText + '<br />' + mobileListLayoutParams.totalProductPriceText + ': ' + cartPrice);
    };
    ...
    // when remove product from gift box is clicked.
